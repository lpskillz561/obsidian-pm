...
- Hand a task to a local Claude Code agent: assign it (default name `claude-zixi`) or pick "Ask Claude" from the task menu, and it investigates the mapped repo and posts its findings back as a comment on that card. The board shows a pulsing badge while a run is in flight. Desktop only — on mobile the feature is simply absent. Configure the binary path, config directory and per-project repository under the new "Claude agent" settings section. The agent runs with a read-only tool allowlist plus exactly one command that can write: the script that appends the comment
- Task notes now carry a managed `## Comments` section, parsed back into the task and preserved across every save

### Fixed

- **Data loss:** text typed below the generated `## Subtasks` checklist in a task note was silently deleted the next time that task was saved from the editor. The strip that removes auto-generated content ran from the `## Subtasks` heading to end-of-file rather than stopping after the generated checklist lines, and opening the task modal loaded that already-truncated body into memory, so the next save wrote the truncation to disk permanently. Only tasks with subtasks were affected, and only via the task editor — board drag-and-drop writes frontmatter only. Text below the checklist now survives, though it moves above the `Project:` link when the note is next rewritten
