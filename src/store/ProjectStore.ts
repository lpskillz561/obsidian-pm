...
import type { Project, StatusConfig, Task, TaskComment } from '../types'
...
function patchNeedsBodyRewrite(patch: Partial<Task>): boolean {
  ...
  // `comments` is deliberately absent: the body rewrite re-reads them from disk
  // (see adoptDiskComments), so patching them here would achieve nothing.
  return patch.description !== undefined || patch.archived !== undefined || patch.subtasks !== undefined
}

/**
 * Load a task note's body into the task: description and managed comments both.
 * Every site that recovers body content from disk goes through here, so a task
 * can never end up holding one half of the body and a stale copy of the other.
 */
function applyBody(task: Task, body: string): void {
  const { description, comments } = parseTaskBody(body)
  task.description = description
  task.comments = comments
}

/**
 * On a body rewrite, the comments on disk win over the ones in memory.
 *
 * Nothing in the plugin authors comments — the agent appends them out of band,
 * straight to the note. So any in-memory copy is only ever as fresh as the last
 * read, and a task modal opened before a run finishes would otherwise serialize
 * its stale clone back over a comment that landed while it was open. Disk is
 * the single writer, so disk is the source of truth.
 */
...
export class ProjectStore {
  ...
  async loadTaskBody(task: Task): Promise<void> {
    ...
    const content = await this.app.vault.cachedRead(file)
    applyBody(task, parseFrontmatter(content).body)
    this.hydratedBodies.add(task)
  }

  /**
   * Re-read a task's body from disk even if it is already hydrated.
   *
   * Needed when something outside the plugin appended to the note — an agent
   * writing a comment, say. Without this the in-memory task still holds the
   * pre-write body, and the next full rewrite would serialize it back over the
   * new content.
   */
  async reloadTaskBody(task: Task): Promise<void> {
    ...
    try {
      ...
      if (existing instanceof TFile) {
        ...
        await this.app.vault.process(existing, (content) => {
          const onDisk = parseTaskBody(parseFrontmatter(content).body)
          if (!this.hydratedBodies.has(task)) task.description = onDisk.description
          adoptDiskComments(task, onDisk.comments)
        ...
        if (previousPath) {
          ...
          if (oldFile instanceof TFile) {
            const onDisk = parseTaskBody(parseFrontmatter(await this.app.vault.cachedRead(oldFile)).body)
            if (!this.hydratedBodies.has(task)) task.description = onDisk.description
            adoptDiskComments(task, onDisk.comments)
          }
        }
