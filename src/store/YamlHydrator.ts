import type { CustomFieldDef, Project, SavedView, Task, TaskType, ViewMode } from '../types'
import { makeTask } from '../types'
import { parseTaskBody } from './YamlParser'

const TASK_TYPES: TaskType[] = ['task', 'bug', 'milestone', 'subtask']

/** Unknown or missing frontmatter types fall back to a plain task. */
function hydrateType(raw: string): TaskType {
  return TASK_TYPES.includes(raw as TaskType) ? (raw as TaskType) : 'task'
}
...
export function mapRawToTask(r: Record<string, unknown>, overrides?: Partial<Task>): Task {
  return makeTask({
    ...
    type: hydrateType(r.type as string),
